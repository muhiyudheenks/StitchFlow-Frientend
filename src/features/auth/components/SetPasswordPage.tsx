...
export default function SetPasswordPage({ token }: SetPasswordPageProps) {
    ...
    useEffect(() => {
        ...
        const checkToken = async () => {
            try {
                ...
                const msg = err.response?.data?.message || 'Invalid or expired setup token link.';
                setTokenError(msg);
            } finally {
                setIsVerifyingToken(false);
            }
    ...
    >({
        ...
        onSuccess: (data) => {
            setSuccessMsg(data?.message || 'Password set successfully! Redirecting to login...');
            setTimeout(() => {
                router.push('/login');
            }, 2500);
        },
                ...
                <div className="hidden lg:flex lg:w-[45%] flex-col justify-between p-12 lg:p-16 bg-gradient-to-br from-slate-900 via-indigo-950 to-purple-950 text-white relative overflow-hidden border-r border-white/10">
                    ...
                    >
                        ...
                        <p className="text-base text-slate-300 leading-relaxed mb-10">
                            Create a secure password to activate your workforce account and access your manufacturing workstation.
                        </p>

                        <div className="space-y-4">
                            {[
                                { title: 'Personal Invitation', desc: 'Secure invitation created by your Administrator' },
                                { title: 'Encrypted Passwords', desc: 'Protected by enterprise bcrypt encryption' },
                                { title: 'Instant Activation', desc: 'Immediate access upon password creation' }
                            ].map((feature, idx) => (
                                <motion.div
                                    key={feature.title}
                    ...
                    >
                        {isVerifyingToken ? (
                            <div className="py-12 text-center text-slate-500">
                                <span className="w-8 h-8 rounded-full border-3 border-purple-600 border-t-transparent animate-spin inline-block mb-4" />
                            ...
                            <div className="text-center py-4">
                                <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-rose-100 text-rose-600 shadow-lg shadow-rose-500/20">
                            ...
                            <>
                                <div className="mb-8">
                                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-50 border border-purple-200 text-purple-700 text-xs font-bold mb-3">
                                        <span>Role: {userInfo?.role.toUpperCase()}</span>
                                    </div>
                                    <h2 className="text-3xl font-extrabold text-slate-900 tracking-tight">
                                        Create Password
                                    </h2>
                                    <p className="text-sm text-slate-500 mt-2">
                                        Welcome <span className="font-bold text-slate-800">{userInfo?.fullName}</span> ({userInfo?.email}). Choose a password to activate your account.
                                    </p>
                                </div>
                                ...
                                <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-5">
                                    ...
                                    {/* New Password */}
                                    <div>
                                        <label className="block text-xs font-extrabold uppercase tracking-wider text-slate-700 mb-2" htmlFor="sp-password">
                                            New Password
                                        </label>
                                    ...
                                    {/* Confirm Password */}
                                    <div>
                                        <label className="block text-xs font-extrabold uppercase tracking-wider text-slate-700 mb-2" htmlFor="sp-confirm">
                                            Confirm Password
                                        </label>
                                        <div className="relative">
                                            <FiLock className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-lg" />
