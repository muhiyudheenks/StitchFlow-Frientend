...
// import { authRequestStarted, authRequestFailed, signInSucceeded } from '@/store/slices/authSlice';
...
//             dispatch(signInSucceeded({ fullName: email.split('@')[0], email }));
//             router.push('/otp');
//         } catch {
...
//     return (
//         // h-screen + overflow-hidden ==> viewport-nu purathu poakilla, scroll varilla
//         <div className="flex h-screen items-center justify-center overflow-hidden bg-gradient-to-b from-[#eef0fd] via-[#eef3fb] to-[#d7f3ec] px-6 py-3">
//             {/* content viewport-nekkal koodiyaal, ee wrapper mathram scroll cheyyum (card cut aakilla) */}
...
//                         <div className="mb-2 flex items-center gap-2 text-xl font-extrabold text-blue-600">
//                             <svg
//                                 className="h-6 w-6"
//                                 viewBox="0 0 24 24"
//                                 fill="none"
...
//                                 className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-11 pr-4 text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
//                             />
//                         </div>
...
//                                 className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-11 pr-11 text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
//                             />
//                             <button
//                                 type="button"
...
//                     <Link href="/create-account" className="font-semibold text-blue-600">
//                         Create account
//                     </Link>
//                 </p>
//             </div>
...
import { useAppDispatch, useAppSelector } from '@/store/hooks';
import { authRequestStarted, authRequestFailed, signInSucceeded } from '@/store/slices/authSlice';
import { signInSchema, type SignInFormValues } from '@/utils/authSchema';
...
export default function SignInForm() {
    ...
    const onSubmit = async (data: SignInFormValues) => {
        ...
        try {
            // TODO: replace with real auth call
            await new Promise((resolve) => setTimeout(resolve, 600));
            dispatch(signInSucceeded({ fullName: data.email.split('@')[0], email: data.email }));
            router.push('/');
        } catch {
            dispatch(authRequestFailed('Something went wrong. Please try again.'));
        }
